...
from src.model_hourglass.model.net_hg import Net_HM_HG
from src.model_hourglass.data.build import build_dataset

from src.model_hourglass.util.net_util import load_net_model
from src.model_hourglass.util.image_util import BHWC_to_BCHW, normalize_image
import time
...
def main():
    ...
    optimizer = optim.RMSprop(net_hm.parameters(), lr=cfg.TRAIN.LR)
    ...
    for epoch in range(num_epoch):
        ...
        if (total_loss_train < minloss):
            torch.save(net_hm.state_dict(), "weights/posenetwork/net_hm.pth")
